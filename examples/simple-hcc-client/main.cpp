...
int main(int argc, char **argv) {
    ...
    unique_ptr<NAHCCApiClient> naHCCApiClient = make_unique<NAHCCApiClient>(username, password, clientId, clientSecret);

    try {
        naHCCApiClient->login("read_homecoach");
        json hccData = naHCCApiClient->requestHomeCoachsData();
        list<Station> stations = utils::parseHCCDevices(hccData);
        for (const Station &station: stations) {
            ...
            for (const Module &module: modules) {
                ...
                cout << "Health index: " << measures.mHealthIndex << "\n";
                cout << "================Module end=========================\n";
            }
            cout << "====================Station end========================\n\n";
        }
