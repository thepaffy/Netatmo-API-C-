...
vector<Module> listToVector(list<Module> &&modules) {
    std::vector<Module> v{ std::make_move_iterator(std::begin(modules)),
                      std::make_move_iterator(std::end(modules)) };
    return v;
}

TEST(ParseDevicesTest, parseWSDevices) {
    ...
    list<Station> stations = parseWSDevices(response);
    EXPECT_EQ(1, stations.size());
    for (const Station &station: stations) {
        EXPECT_STREQ("70:ee:50:29:48:4e", station.id().c_str());
        EXPECT_STREQ("Over", station.name().c_str());
        vector<Module> modules = listToVector(station.modules());
        ...
        EXPECT_STREQ("Wohnzimmer", module.name().c_str());
        EXPECT_STREQ("NAMain", module.type().c_str());
        measures = module.measures();
        EXPECT_EQ(1509446950, measures.mTimeStamp);

        module = modules.at(1);
        EXPECT_STREQ("02:00:00:29:2c:7a", module.id().c_str());
        EXPECT_STREQ("Aussenraum", module.name().c_str());
        EXPECT_STREQ("NAModule1", module.type().c_str());
        measures = module.measures();
        EXPECT_EQ(1509446923, measures.mTimeStamp);

        module = modules.at(2);
        EXPECT_STREQ("05:00:00:04:53:b2", module.id().c_str());
        EXPECT_STREQ("Niederschlagsmesser", module.name().c_str());
        EXPECT_STREQ("NAModule3", module.type().c_str());
        measures = module.measures();
        EXPECT_EQ(1509446936, measures.mTimeStamp);

        module = modules.at(3);
        EXPECT_STREQ("06:00:00:01:d9:f2", module.id().c_str());
        EXPECT_STREQ("Windmesser", module.name().c_str());
        EXPECT_STREQ("NAModule2", module.type().c_str());
        measures = module.measures();
        EXPECT_EQ(1509446827, measures.mTimeStamp);
    }
}
